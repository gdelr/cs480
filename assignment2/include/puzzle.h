...
public:
  ...
  int sumOftheDistancesOfEveryTileToItsGoalPosition(){
    int sum=0;
    for(int i=0;i<5;i++){
      for(int j=0;j<5;j++){
	if(currentState[i][j]!=goalState[i][j]){
	  if(currentState[i][j]==0){
	    //in a sence the 0 space would be space 25
	    sum+=abs(goalState[i][j]-25);
	  }
	  else
	    sum+=abs(goalState[i][j]-currentState[i][j]);
	}
      }
    }
    return sum;
